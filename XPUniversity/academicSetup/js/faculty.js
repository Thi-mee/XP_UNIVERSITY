...
const addFaculty = async (e) => {
  ...
  try {
    const res = await axios.post('http://localhost:8097/api/v1/faculties/add', data)
...
const deleteFaculty = (index) => {
  ...
  const faculty = faculties[index];
  try {
    axios.delete(`http://localhost:8097/api/v1/faculties/${faculty.FacultyId}`)
...
const updateFaculty = async (index) => {
  ...
  try {
    await axios.put(`http://localhost:8097/api/v1/faculties`, data);
...
const search = async (e) => {
  ...
  try {
    const res = await axios.post('http://localhost:8097/api/v1/faculties', data);
